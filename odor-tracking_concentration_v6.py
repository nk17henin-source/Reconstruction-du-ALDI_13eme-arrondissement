...
# Paramètres communs
d = 7              # Longueur du "Surge" (remontée face au vent quand odeur détectée)
...
plt.scatter(x_pts*dl, y_pts*dl-h, c='k', s=5, alpha=0.6, label='Odor')
...
if len(trajet_sonde) > 0:
    ...
    plt.plot(trajet_x, trajet_y, 'r.-', linewidth=1, markersize=3, label='Trajectory')
    plt.scatter([trajet_x[0]], [trajet_y[0]], c='red', s=120, marker='*', zorder=10, label='Start')
...
plt.title(f"Simulation Tracking - Stategy : {strat.upper()} (d={d})")
plt.xlim(0, l)
plt.ylim(-h, h)
plt.tight_layout()
